...
//////// Fortaleza
const LAT = -3.7304, LON = -38.5218, TZ = -3;
const $ = (id)=>document.getElementById(id);
const ui = { date: $('date'), time: $('time'), hpost: $('hpost'), outAz: $('outAz'), outEl: $('outEl'), outLen: $('outLen') };

//////// UI Helpers
function todayStr() {
  const utc = Date.now() - (3 * 60 * 60 * 1000);
...
function readLocalParts() {
  const d = ui.date.value ? ui.date.value : todayStr();
...
function resetCamera() {
  const r = 10;
  const h = parseFloat(ui.hpost.value) || 1;
  const theta = Math.PI;
  const phi = Math.PI / 6;
...
function setShadow(len, bearingDeg){
  ...
  const y = 0.01; // ligeiro offset para evitar z-fighting
...
function update() {
  ...
  const sp = solarAzElNoaa(Y, M, D, hh, mm, 0, LAT, LON, TZ);
...
['input', 'change'].forEach(ev => { [ui.date, ui.time, ui.hpost].forEach(el => el.addEventListener(ev, update)) });
...
//////// Checks
// (1) Three carregado
console.assert(!!THREE && !!THREE.Scene, 'THREE não carregado');
// (2) OrbitControls disponível
console.assert(typeof OrbitControls === 'function', 'OrbitControls não disponível');
